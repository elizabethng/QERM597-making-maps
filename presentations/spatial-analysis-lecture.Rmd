...
```{r}
(q1 <- q0 +
  coord_sf(ylim = c(-20, -18.5), crs = st_crs(4326)) + # make space for legend
  scale_color_manual(
  ...
  theme_bw() +
  theme(legend.position = c(0.78, 0.15),       # position legend in lower right corner
        legend.key = element_rect(fill = NA),  # remove background of legend keys
        panel.grid.major = element_line(color = "white")) + # remove graticules but leave coordinates
...
## Natural earth package to access other data
Create a base map with some information
```{r}
b0 <- ggplot(data = ne_africa) +  # need to put data in ggplot call for geom_rect to work
  geom_sf(fill = "gray93", color = "gray93") + 
  geom_sf(data = ne_namibia, fill = "gray78", color = "gray93") +
  geom_sf(data = africa_rivers, color = "lightskyblue") +
  theme_bw() +
  coord_sf(xlim = c(-35, 51),       
           ylim = c(-46.96, 37.34),  # make additional space for inset map
...
```{r}
b1 <- b0 +
  geom_rect(xmin = 14, xmax = 19, ymin = -21, ymax = -17, 
