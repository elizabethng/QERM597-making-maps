...
---
```{r}
Namibia <- st_read(here("data", "NAM-level_1.shp"))
class(Namibia)
head(Namibia)
...
## Reading raster data
```{r echo = FALSE, fig.align="center"}
#Reading raster data into workspace
raster(here("data", "ndvi_mean_utm.tif"))#From a file in your directory
raster(here("data", "ndvi_mean_utm.tif"))#Other method
raster(here("data", "ndvi_mean_utm.tif"))#Yet another method
```
...
NDVI_mean <- raster(here("data", "ndvi_mean_utm.tif")) * 0.0001
NDVI_mean
...
```{r}
# Natural earth 1 with shaded relief and drainages
# address <- "https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/raster/NE1_HR_LC_SR_W_DR.zip"
...
# ne_basemap <- ne_download(scale = "large", 
#                           type = "NE1_HR_LC_SR_W_DR", 
#                           category = "raster", 
#                           returnclass = "sf")

ne_namibia <- ne_countries(scale = "medium", country = "namibia", returnclass = "sf")
ne_africa <- ne_countries(scale = "medium", continent = "africa", returnclass = "sf")
ne_feature <- ne_download(scale = 10, category = "physical", type = "rivers_lake_centerlines", returnclass = "sf") 
africa_rivers <- st_intersection(ne_feature, ne_africa)

ggplot() + 
  geom_sf(data = ne_africa) + 
  geom_sf(data = ne_namibia, fill = "grey") +
...
q <- ggplot() +
  ...
  geom_sf(data = zebra_sf, aes(color = ID), alpha = 0.1, inherit.aes = FALSE, show.legend = "point") +
  scale_color_manual(values = viridis::magma(length(unique(zebra_sf$ID)))) +
  theme(legend.key = element_rect(fill = NA)) + # remove background of legend keys
  guides(color = guide_legend(override.aes = list(alpha = 1))) # make the legend keys 100% opaque
q
...
q0 <- ggplot() +
  geom_sf(data = large_roads, size = 1.5, color = "grey", inherit.aes = FALSE) + 
  geom_sf(data = small_roads, size = 0.6, color = "grey", inherit.aes = FALSE) + 
  geom_sf(data = zebra_sf, aes(color = large_road_dist/1000), alpha = 0.1, inherit.aes = FALSE) +
...
q1 <- q0 +
  coord_sf(ylim = c(-20, -18), crs = st_crs(4326)) +
...
world_points<- st_centroid(zebra_sf)
world_points <- cbind(zebra_sf, st_coordinates(st_centroid(zebra_sf$geometry)))

# zebra_point <- st_centroid(zebra_sf[1,])
# zebra_point <- cbind(zebra_point, st_coordinates(st_centroid(zebra_point$geometry)))

ggplot() + 
  geom_sf(data = zebra_sf) +
  geom_text(data = zebra_point, aes(x = X, y = Y, label = ID),
